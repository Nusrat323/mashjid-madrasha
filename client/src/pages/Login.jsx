      ...
      <div className="relative hidden overflow-hidden bg-linear-to-br from-indigo-900 to-indigo-950 lg:block">
        ...
        >
          ...
          <p className="mt-4 max-w-md text-lg text-indigo-100">
            লগইন করে নিরাপদে দান করুন এবং আপনার সব দানের তালিকা যেকোনো সময় দেখে নিন।
          </p>
        </motion.div>
      </div>
      ...
      <div className="flex items-center justify-center px-4 py-16">
        ...
        >
          ...
          <p className="mt-2 text-slate-600">
            {mode === "login" ? "আপনার অ্যাকাউন্টে লগইন করুন।" : "কয়েক সেকেন্ডেই অ্যাকাউন্ট তৈরি হয়ে যাবে।"}
          </p>
            ...
            {[
              ...
              <button
                ...
                className={`rounded-lg py-2.5 text-sm font-semibold transition ${
                  mode === value ? "bg-white text-indigo-900 shadow-sm" : "text-slate-500"
                }`}
              >
                {label}
              </button>
          ...
          >
            <FaGoogle className="text-rose-500" /> গুগল দিয়ে চালিয়ে যান
          </button>
          ...
          <form onSubmit={submit} className="space-y-4">
            {mode === "register" && (
              <Field label="আপনার নাম">
                <input required value={form.name} onChange={(e) => set("name", e.target.value)} className={inputClass} placeholder="পূর্ণ নাম" />
              </Field>
            )}
            <Field label="ইমেইল">
              <input required type="email" value={form.email} onChange={(e) => set("email", e.target.value)} className={inputClass} placeholder="example@email.com" />
            </Field>
            <Field label="পাসওয়ার্ড">
              <input required type="password" minLength={6} value={form.password} onChange={(e) => set("password", e.target.value)} className={inputClass} placeholder="কমপক্ষে ৬ অক্ষর" />
            </Field>
